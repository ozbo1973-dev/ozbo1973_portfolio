...
const { mockVerifySession, mockCreateUserReply, mockSendReplyNotification } =
  vi.hoisted(() => ({
...
describe("createUserReplyAction", () => {
  ...
  it("returns success with reply when input is valid", async () => {
    const result = await createUserReplyAction(
      "root-1",
  ...
  it("rejects unauthenticated caller by re-throwing redirect", async () => {
    ...
    await expect(createUserReplyAction("root-1", "Hello")).rejects.toThrow(
      "NEXT_REDIRECT",
    );
  });
  ...
  it("returns error when ownership check fails (DAL throws)", async () => {
    mockCreateUserReply.mockRejectedValue(
      new Error("Submission not found or does not belong to user."),
    );
